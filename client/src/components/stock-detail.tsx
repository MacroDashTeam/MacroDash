        ...
        {aiInsightsData?.insights && aiInsightsData.insights.length > 0 && (
          <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-6">
            ...
            <div className="mt-4 text-xs text-zinc-500 text-center">
              Insights generated by AI • Updated every 6 hours
            </div>
          </div>
        )}
              ...
              {news.map((article: any, index: number) => {
                ...
                return (
                  <a
                    ...
                    className="flex flex-col p-5 rounded-lg border border-zinc-800 hover:border-zinc-700 hover:bg-zinc-900/80 transition-all hover:shadow-lg group h-full"
                  >
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <h3 className="font-semibold text-white line-clamp-3 flex-1 group-hover:text-blue-400 transition-colors">{article.title}</h3>
                      <ExternalLink className="w-4 h-4 text-zinc-500 flex-shrink-0 mt-1" />
                    </div>
                    <p className="text-sm text-zinc-400 mb-4 line-clamp-3 flex-1">{article.summary}</p>
